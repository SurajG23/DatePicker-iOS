...
extension WeeksPicker: UIPickerViewDelegate {
    public func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if weeks[row].lastDay >= currentDate {
            if self.isSelectFutureWeek {
                ...
                self.firstDay = currentDate.startOfWeek!
                self.lastDay = currentDate.endOfWeek!
                self.selectedWeek = CalendarFormatter.weekDateFormatter.string(from: currentDate.startOfWeek!) + " - " + CalendarFormatter.weekDateFormatter.string(from: currentDate.endOfWeek!)
            }
        } else {
            self.date = weeks[row].firstDay
            self.firstDay = weeks[row].firstDay
